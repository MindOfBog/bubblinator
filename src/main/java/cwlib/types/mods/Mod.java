...
public class Mod extends FileDB  {
    ...
    public static Mod fromLegacyMod(File file) {
        ...
        if (type.equals("e")) {
            ResourceSystem.println("Mod", "Mod is encrypted! Attempting to decrypt using default passphrase.");
            if (!stream.bool()) {
                stream = new MemoryInputStream(Crypto.decrypt(Mod.LEGACY_PASSWORD, stream.bytes(stream.getLength() - 5)), CompressionFlags.USE_ALL_COMPRESSION);
            ...
            } else throw new SerializationException("Custom keyed mods are no longer supported.");
        ...
        for (int i = 0; i < patchCount; ++i) {
            ...
            if (patchType != LAMS_TYPE)
                throw new SerializationException("Only LAMS patches are supported on legacy mods!");
    ...
    public ModInfo getConfig() { return this.config; }
    public ArrayList<ModPatch> getPatches() { return this.patches; }
